...
// Verifies the Turnstile token with Cloudflare's server.
// Returns true if genuine browser, false if bot or expired token.
// This call never touches your DB — it's a single fetch to Cloudflare's API.
async function verifyTurnstile(token, ip) {
  try {
    const res = await fetch(
      "https://challenges.cloudflare.com/turnstile/v0/siteverify",
      {
        ...
        body: JSON.stringify({
          ...
          response: token,
          // Passing the IP adds an extra layer — Cloudflare checks the token
          // was issued to the same IP that is now submitting the form.
          remoteip: ip,
        }),
      },
    );
    ...
    // If Cloudflare is unreachable, fail open so genuine users aren't blocked.
    // Change to `return false` if you want to block on Cloudflare outage.
...
export async function POST(request) {
  try {
    ...
    // ── Honeypot Check ────────────────────────────────────────────────────────
    // The `website` field is hidden in the UI — real users never see or fill it.
    // Bots crawl the DOM and fill every input they find.
    // We return 200 (not 400) so bots think they succeeded and stop retrying.
    if (body.website) {
    ...
    // Verify the token Cloudflare issued to this browser on the frontend.
    // If missing or invalid, reject before touching the DB or sending email.
    const cfToken = body.cfToken;
    if (!cfToken) {
      return errorResponse("Bot verification token missing.", 400);
    }
